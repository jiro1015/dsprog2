...
class CalculatorApp(ft.Container):
    ...
    def button_clicked(self, e):
        ...
        # ★ 科学計算ボタン
        elif data in ("sin", "cos", "tan", "log", "ln", "x²", "√x"):
            try:
                x = float(self.result.value)
                if data == "sin":
                    self.result.value = self.format_number(math.sin(math.radians(x)))
                elif data == "cos":
                    self.result.value = self.format_number(math.cos(math.radians(x)))
                elif data == "tan":
                    self.result.value = self.format_number(math.tan(math.radians(x)))
                elif data == "log":
                    self.result.value = self.format_number(math.log10(x))
                elif data == "ln":
                    self.result.value = self.format_number(math.log(x))
                elif data == "x²":
                    self.result.value = self.format_number(x * x)
                elif data == "√x":
                    if x < 0:
                        self.result.value = "Error"
                    else:
                        self.result.value = self.format_number(math.sqrt(x))
